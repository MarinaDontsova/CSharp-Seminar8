...
bool FindElementInCube(int[,,] cube, int rand)
{
    for (int i = 0; i < cube.GetLength(0); i++)
    {
        for (int j = 0; j < cube.GetLength(1); j++)
        {
            ...
            {
                if (cube[i, j, k] == rand) return true; // если число найдено, то передаем Истина
            }
        }
    }
    return false; // если число не найдено, то передаем Ложь
}
    ...
    Console.WriteLine("Введите данные для создания трёхмерного массива:");
